...
7. Keep Harness headless by default. Share the returned `webUrl` as a clickable link without calling `open_run`, setting `openBrowser: true`, or opening the OS browser. Call `open_run` only when the user explicitly asks to open or show the Harness page. Browser verification is required only for that explicit open request; it is never a prerequisite for sharing the stable URL or monitoring the run.
