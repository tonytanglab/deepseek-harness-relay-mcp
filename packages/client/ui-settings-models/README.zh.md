...
凭据提供方公开全局和项目作用域时，编辑器会同时显示两者。已配置的密钥选中当前生效的受管作用域，未配置的密钥采用提供方默认作用域。把项目覆盖切换为全局时，系统会先提交共享值，再移除该项目的覆盖，因此全局写入失败不会丢失当前生效的密钥。删除提供方 profile 时，系统不会删除由其他项目共用的用户级全局凭据。
