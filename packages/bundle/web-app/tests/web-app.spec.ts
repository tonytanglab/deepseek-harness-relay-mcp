...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and prints the URL with the LAN snapshot', async () => {
    ...
    apply(ctx, new Config({ printUrl: true, readyFormat: 'text', surfaceContext: true, trustedHosts: ['lab.internal'] }))
  ...
  it('stays quiet with printUrl off', async () => {
    ...
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ printUrl: false, readyFormat: 'text', surfaceContext: true, trustedHosts: [] }))
    await ctx.plugin(SystemPrompt, { persona: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it('skips the surface context when disabled (the one-shot layer): no prompt section, no bash variables', async () => {
    ...
    apply(ctx, new Config({ printUrl: false, readyFormat: 'text', surfaceContext: false, trustedHosts: [] }))
    await ctx.plugin(SystemPrompt, { persona: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
    const assembly = await ctx.systemPrompt.assemble()
  ...
  it('prints the loopback-only URL line when no LAN snapshot exists', async () => {
    ...
    ctx.provide('webServer', fakeHttpServer().server)
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ printUrl: true, readyFormat: 'text', surfaceContext: true, trustedHosts: [] }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('dsh web: http://127.0.0.1:4567')
    await ctx.fiber.dispose()
  })

  it('prints one stable JSON readiness record for a supervisor', async () => {
    stageDist()
    const ctx = new Context()
    ctx.provide('webServer', fakeHttpServer().server)
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ printUrl: true, readyFormat: 'json', surfaceContext: true, trustedHosts: [] }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('{"type":"dsh/web-ready","url":"http://127.0.0.1:4567"}')
    await ctx.fiber.dispose()
  })
  ...
  it('defers the URL line until Loader settlement and drops it on failure or teardown', async () => {
    ...
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(settled, new Config({ printUrl: true, readyFormat: 'text', surfaceContext: true, trustedHosts: [] }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).not.toHaveBeenCalled()
    release!()
    await new Promise(resolve => setTimeout(resolve, 0))
    ...
    apply(failed, new Config({ printUrl: true, readyFormat: 'text', surfaceContext: true, trustedHosts: [] }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).not.toHaveBeenCalled()
    ...
    apply(torn, new Config({ printUrl: true, readyFormat: 'text', surfaceContext: true, trustedHosts: [] }))
