...
文件型[凭据](../credentials/README.md)提供方。普通 Harness home 保持原有四层行为；受监管的 Codex 服务会在项目覆盖之下增加一份用户级全局受管默认文档。

| 层 | 来源 id | 可写 | 优先 |
|---|---|---|---|
| 继承的进程环境 | `env` | 否 | 始终优先 |
| 项目凭据文档 | `project-file`（未配置 `globalPath` 时为 `file`） | 是（`set`/`unset`） | 高于全局文档和两个 `.env` 层 |
| 可选的用户级全局文档 | `global-file` | 是（`set`/`unset`） | 高于两个 `.env` 层 |
...
两个受管文档之下的所有来源优先级都低于它们，因此 Models 页写入的密钥会立即生效，即使某个 `.env` 里还留着更旧的密钥。同一引用存在项目文档时，它会覆盖全局默认。未配置 `globalPath` 时，项目文档仍使用来源 id `file` 并保持原有的无值 `describe()` 字段；配置后，`describe()` 还会报告可写作用域、默认写入作用域和当前生效的受管作用域。
...
| `globalPath` | 未设置 | 位于项目覆盖之下的可选用户级全局凭据文档。 |
| `defaultScope` | `project` | 未指定作用域时的写入目标；`global` 要求同时配置 `globalPath`。 |
| `watch` | `true` | 热发布外部编辑。 |
...
写入是对选定的已解析文档打补丁而不是重建，因此注释与所有未触及条目的排版都会保留。直接位于某条目上方的注释属于该条目的注解，会随它一起删除。每次写入都先在 [`dsh-atomic-write`](../../util/atomic-write/README.md) 的跨进程写锁下重读该文档、把此前未观察到的一切发布出去，再在仅属主可访问（`0700`）的目录下以 `0600` 权限原子提交——因此并发写入者、或落在 watcher 防抖窗口内的外部编辑会被并入，而不是被覆盖。磁盘上已经无法解析的文档会让写入失败，而不是覆盖提供方读不懂的内容。
...
两份受管文档的外部编辑都会在对应快照**整体替换**后按变更引用逐个发布 `credentials/updated`——磁盘上删掉的条目绝不在内存滞留。在 Chokidar 打开每个目标之前，提供方会对层级最深的现有祖先路径执行 realpath 解析，再拼回缺失的后缀；文件访问和诊断仍使用配置路径，从而避免 Windows 混用 8.3 别名与 libuv 的长格式事件路径。提供方自己的写入按内容识别，只发布属于该次提交的一个事件。运行期文档不可读或无效时保留其最后可用快照并告警；文件不存在即空存储；启动时不可读或无效则明确报错。
